...
adne <- read.csv("04_Analyses_prokaryotes/00_data/matrice_prokaryote_totale.csv", header=T, row.names=1)
adne <- adne[rowSums(adne)!=0,]
adne <- adne[,colSums(adne)!=0]

proka <- as.data.frame(t(adne))
...
proka2plot <-left_join(proka2plot, meta_tot[,c("code_spygen", "site", "habitat")])
...
proka_plot <- ggplot(proka2plot, aes(x=A1, y=A2))+
  ...
  ggtitle("PCoA prokaryote taxa composition")

proka_plot

ggsave("04_Analyses_prokaryotes/03_Outputs/PCoA_proka_tous_sites.png", width = 11, height = 8)
