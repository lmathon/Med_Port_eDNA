...
###COMPARAISON HABITAT PORT VS HABITAT BIOHUT

# Metadata
meta <- read.csv("00_Metadata/metadata_port.csv", header=T)

# Enlever ceux sans biohuts
meta <- meta %>%
...
grp1.hull<-seg.data[seg.data$group=="Port",1:3][chull(seg.data[seg.data$group=="Port",2:3]),]
grp2.hull<-seg.data[seg.data$group=="BIOHUT_port",1:3][chull(seg.data[seg.data$group=="BIOHUT_port",2:3]),]
...
panel.a<-ggplot() + 
  ...
  theme_mine + 
  scale_colour_manual(values=c("steelblue3","darkorange"), labels = c("Biohut","Port")) +
  scale_shape_manual(values=c(19,17), labels = c("Biohut","Port")) +
  labs(title=" ",x="PCoA 1",y="PCoA 2",
       colour = "Habitat", shape = "Habitat") 

  panel.a

ggsave("04_Analyses_prokaryotes/03_Outputs/PCoA_biohut_port.png", width = 11, height = 8)
