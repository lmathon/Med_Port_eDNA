...
# load data port
species_port <- read.csv("01_Analyses_teleo/00_data/reduced_matrice_teleo_port.csv", row.names=1) %>%
  row.names(.) %>%
  as.data.frame(.)
...
fam_proportion <- data.frame(Family=character(94), port=numeric(94), outside=numeric(94), both=numeric(94))
...
write.csv(fam_proportion, file="01_Analyses_teleo/02_Analyses/Analyses_reduced_species_list/Outputs/count_families.csv", row.names = F)
