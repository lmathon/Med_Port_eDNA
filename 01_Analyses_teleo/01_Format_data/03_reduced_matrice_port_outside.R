...
species_expert <- read.csv("01_Analyses_teleo/00_data/ListePoissonsExpert.csv", header=F)

adne_port <- adne_port %>%
  filter(Species %in% species_expert$V1)

rownames(adne_port) <- adne_port$Species
adne_port <- adne_port[,-1]

write.csv(adne_port, "01_Analyses_teleo/00_data/reduced_matrice_teleo_port.csv")
...
write.csv(adne_tot, "01_Analyses_teleo/00_data/reduced_matrice_teleo_totale.csv") 
